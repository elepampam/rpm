...
<body>
	<div class="container">
        ...
        <ul class="nav nav-tabs">
            <li class="active"><a href="<?php echo site_url(); ?>/admin/debit">DEBIT</a></li>
            <li><a href="<?php echo site_url(); ?>/admin/kredit">KREDIT</a></li>
        </ul>
        ...
        reader.onload = function(e) {
            ...
            for (var i = 1; i < csvval.length; i++) {
                ...
                for (var j = 0; j < fakturValue.length; j++) {
                    fakturValue[j] = fakturValue[j].replace('"', '');
                    if (j == 11) {
                        hitungPpn(parseInt(fakturValue[j]));
                    } else if (j == 12) {
                        hitungPpnBm(parseInt(fakturValue[j]));
                    }
                    if (j == 10 || j == 11 || j == 12) {
                        fakturValue[j] = toRp(fakturValue[j]);
                    }
                    row = row + "<td>" + fakturValue[j] + "</td>";                    
                }
                table = table + row + "</tr>";
            }
            table = table + "</tbody>";
